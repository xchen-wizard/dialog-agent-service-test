...
async def batch_process(batch_data: list[str]):
    return await asyncio.gather(
        *[run_inference(docs=[('inbound', msg)],
                       vendor_name=VENDOR_NAME,
                       merchant_id=MERCHANT_ID,
                       project_id=PROJECT_ID,
                       endpoint_id=ENDPOINT_ID) for msg in batch_data]
    )


if __name__ == '__main__':
    # load data
    dfs = pd.read_excel("/Users/xchen/data/G.O.A.T. Fuel - Sample Questions.xlsx", sheet_name=None, names=['query'], header=None)
    for sheet_name in dfs:
        ...
        dfs[sheet_name] = dfs[sheet_name][(~dfs[sheet_name]['query'].isna()) & (dfs[sheet_name]['query'].str.strip() != '')]
        ...
        try:
            ...
            dfs[sheet_name].to_json("-".join([VENDOR_NAME, sheet_name]), lines=True, orient='records')
    ...
    pd.concat(new_dfs).to_csv('goatfule_sample_questions_responses_v2.csv', index=False)
