...
class T5InferenceService:
    ...
    def infer(self, conversation, vendor, predict_fn):
        ...
        """
        # first predict task
        task = predict_fn(DSTDataset.create_input_task(
            conversation, task_descriptions=self.task_descriptions))[0]

        if task not in {'StartOrBuildOrder', 'FinalizeOrder'}:
            conversation += 'Seller: '
            response = predict_fn(
                self.response_prediction_prompt[vendor] + conversation)[0]
            return {'task': task, 'response': response}
        if task == 'FinalizeOrder':
            return {'task': task}
        if task == 'StartOrBuildOrder':
            products = predict_fn(
                DSTDataset.create_input_cart(conversation))[0]
            if products == 'None':
