...
def get_txt_demo_faqs():
    ...
    lines = open('./under_luna_data/under_luna_faq.txt').readlines()
    for i, lx in enumerate(lines):
        lx = lx.strip()
        part = lx.partition(':')
        if part[0] != 'Question' and part[0] != 'Answer':
            print(f'Error in line {i}: {lx} ({part[0]})')
            sys.exit()
...
def faq_demo(es_client, question: str):
    ...
    if focus is not None:
        ...
        if ' its ' in q1.lower() and focus is not None:
            q1 = q1.replace(' its ', ' ' + focus + ' ')
        elif ' that ' in q1.lower() and focus is not None:
