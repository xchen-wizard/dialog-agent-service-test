...
def handle_create_or_update_order_cart(cnv_obj=None, merchant_id=None, current_cart=None, predict_fn=None, **kwargs):
    ...
    ambiguous_products = [tup[0] for tup in model_predicted_cart if '||' in tup[0]]
    if ambiguous_products:
        response = "\n".join([gen_disambiguation_response_llm(merchant_id, product) for product in ambiguous_products])
