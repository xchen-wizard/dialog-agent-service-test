...
class T5InferenceService:
    ...
    def infer(self, conversation, vendor, merchant_id, predict_fn):
        ...
        if 'AnswerMiscellaneousQuestions' in task and vendor in self.response_prediction_prompt["qa"]:
            conversation += "Seller: "
            ...
            response += predict_fn(self.response_prediction_prompt["qa"][vendor] + qa_prompt + conversation[-MAX_CONVERSATION_CHARS:])[0]
        if 'RecommendProduct' in task and vendor in self.response_prediction_prompt["recommend"]:
            ...
            response += predict_fn(self.response_prediction_prompt["recommend"][vendor] + recommend_prompt + conversation[-MAX_CONVERSATION_CHARS:])[0]
