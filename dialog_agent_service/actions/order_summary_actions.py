...
def create_order_summary_message(cart_id: int):
    ...
    try:
        ...
        logger.error(f'Create order summary message GQL-API request failed: {err}')
        return None
    
    order_summary = resp['createOrderSummaryMessage']
    if not order_summary:
        logger.warn(f'createOrderSummaryMessage failed, no results merchant_id:{cart_id}')
        return None
    
    logger.info(f'merchant_id:{cart_id}, createOrderSummaryMessage results: {order_summary}')
